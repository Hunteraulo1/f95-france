<script lang="ts">
	...
	const p = $derived(variant === 'profile');

	const h1Class = $derived(
		p
	...
	const h4Class = $derived(
		p
			? 'mt-4 mb-1.5 text-base font-semibold leading-snug text-base-content'
			: 'mt-6 mb-2 text-base font-bold leading-tight text-base-content'
	);
	const pClass = $derived(
		p ? 'mb-3 leading-[1.6] text-base-content/90' : 'mb-4 leading-[1.65] text-base-content/85'
	);
	const ulClass = $derived(p ? 'mb-3 list-disc pl-5' : 'mb-4 list-disc pl-6');
	const olClass = $derived(p ? 'mb-3 list-decimal pl-5' : 'mb-4 list-decimal pl-6');
	const hrClass = $derived(p ? 'my-4 border-base-300' : 'my-8 border-base-300');
	const blockquoteClass = $derived(
		p
...
{#snippet inlines(nodes: Inline[])}
	{#each nodes as node, i (i)}
		...
		{:else if node.kind === 'link'}
			<a
				class="text-primary underline underline-offset-2 hover:opacity-85"
				href={node.href}
