...
export const PASSWORD_RESET_THROTTLE_MESSAGE =
	'Trop de demandes de réinitialisation. Réessayez dans quelques minutes.';

/** Limite les demandes de reset mot de passe (par IP). */
export async function checkPasswordResetRequestThrottle(
	event: RequestEvent
): Promise<{ ok: true } | { ok: false; message: string }> {
	const key = scopedClientKey(event, 'password-reset');
	const [row] = await db
		.select()
		.from(table.loginThrottle)
		.where(eq(table.loginThrottle.clientKey, key))
	...
	if (row?.lockedUntil && row.lockedUntil.getTime() > Date.now()) {
		return { ok: false, message: PASSWORD_RESET_THROTTLE_MESSAGE };
	}
	return { ok: true };
}

export async function recordPasswordResetRequest(event: RequestEvent): Promise<void> {
	const key = scopedClientKey(event, 'password-reset');
	const now = new Date();
	const [row] = await db
		.select()
